...
public class ExecutionProofGossipIntegrationTest {
  ...
  @Test
  public void shouldGossipExecutionProofsToPeers() throws Exception {
    final GossipEncoding gossipEncoding = GossipEncoding.SSZ_SNAPPY;

    Set<SignedExecutionProof> receivedExecutionProofs = new HashSet<>();
    final OperationProcessor<SignedExecutionProof> operationProcessor =
        (msg, __) -> {
          receivedExecutionProofs.add(msg);
          return SafeFuture.completedFuture(InternalValidationResult.ACCEPT);
        };

    final Consumer<Eth2P2PNetworkBuilder> networkBuilder =
        b -> b.gossipEncoding(gossipEncoding).executionProofTopicEnabled(true);
    ...
    final Consumer<Eth2P2PNetworkBuilder> networkBuilder2 =
        b ->
            b.gossipEncoding(gossipEncoding)
                .executionProofTopicEnabled(true)
                .gossipedExecutionProofOperationProcessor(operationProcessor);
    ...
    // Publish execution proof
    node1.gossipExecutionProof(signedExecutionProof);

    // Verify the expected proof was gossiped across the network
    Waiter.waitFor(() -> assertThat(receivedExecutionProofs).containsExactly(signedExecutionProof));
  }
